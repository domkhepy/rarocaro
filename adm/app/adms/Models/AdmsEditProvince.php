...
/**
 * A classe AdmsEditProvince recebe as informações que serão editadas no banco de dados
 *
 * @author Domingos
 */
class AdmsEditProvince
{
    ...
    /**
     * Método para fazer busca do Id na tabela sts_province e validar o mesmo
     * @param array $id Recebe a informação que será validada e editada no banco de dados */
    public function viewProvince($id) {
        $this->id = (int) $id;
        $viewProvince = new \App\adms\Models\helper\AdmsRead();
        $viewProvince->fullRead("SELECT id, name
                FROM sts_provinces
                WHERE id=:id 
                LIMIT :limit", "id={$this->id}&limit=1");

        $this->resultadoBd = $viewProvince->getResult();
        if ($this->resultadoBd) {
            $this->resultado = true;
        } else {
    ...
    private function edit() {
        ...
        $upAccessLevel->exeUpdate("sts_provinces", $this->dados, "WHERE id =:id", "id={$this->dados['id']}");

        if ($upAccessLevel->getResult()) {
            $_SESSION['msg'] = "<div class='alert alert-success' role='alert'>Provincia  editada com sucesso!</div>";
            $this->resultado = true;
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Provincia  não editada com sucesso!</div>";
            $this->resultado = false;
        }
    }
