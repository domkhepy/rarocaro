...
/**
 * A classe AdmsAddCategories recebe as informações que serão enviadas para o banco de dados
 *
 * @author Domingos
 */
class AdmsAddCategories
{
    ...
    private function add() {
            ...
            $createAccessLevel->exeCreate("sts_categories", $this->dados);

            if ($createAccessLevel->getResult()) {
                $_SESSION['msg'] = "<div class='alert alert-success' role='alert'>Categoria cadastrada com sucesso!</div>";
                $this->resultado = true;
            } else {
                $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Categoria não cadastrada com sucesso. Tente mais tarde!</div>";
                $this->resultado = false;
            }
