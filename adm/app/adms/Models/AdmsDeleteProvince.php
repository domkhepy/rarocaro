...
/**
 * A classe AdmsDeleteProvince recebe a informação que será deletada do banco de dados
 *
 * @author Celke
 */
class AdmsDeleteProvince
{
    ...
    public function deleteProvince($id) {
        ...
        if ($this->viewProvince() ) {
            $deleteProvince = new \App\adms\Models\helper\AdmsDelete();
            $deleteProvince->exeDelete("sts_provinces", "WHERE id =:id", "id={$this->id}");

            if ($deleteProvince->getResult()) {
    ...
    private function viewProvince() {
        ...
        $viewProvince->fullRead("SELECT id FROM sts_provinces
                WHERE id=:id
