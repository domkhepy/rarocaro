...
class AdmsDeleteProductImages
{
    ...
    /** @return Retorna o resultado do view */
    ...
    public function deleteUser($id) {
        ...
        if ($this->viewUsers()) {
            ...
            $deleteUser->exeDelete("sts_product_images", "WHERE id =:id", "id={$this->id}");
            ...
            if ($deleteUser->getResult()) {
                ...
                $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Imagem não apagada com sucesso!</div>";
    ...
    private function viewUsers() {
        ...
        $viewUser->fullRead("SELECT id, name image, sts_product_id product_id 
        ...
        if ($this->resultadoBd) {
            ...
            $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Imagem não encontrada!</div>";
            return false;
        }
    }
    ...
    private function deleteImg() {
        if ((!empty($this->resultadoBd[0]['image'])) OR ($this->resultadoBd[0]['image'] != null)) {
            $this->delDiretorio = "app/adms/assets/image/products/" . $this->resultadoBd[0]['product_id'];
