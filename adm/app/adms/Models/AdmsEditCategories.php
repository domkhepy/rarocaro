...
/**
 * A classe AdmsEditCategories recebe as informações que serão editadas no banco de dados
 *
 * @author Domingos
 */
class AdmsEditCategories
{
    ...
    /**
     * Método para fazer busca do Id na tabela sts_categories e validar o mesmo
    ...
    public function viewCategories($id) {
        ...
        $viewCategories->fullRead("SELECT id, name
                FROM sts_categories
                WHERE id=:id 
    ...
    private function edit() {
        ...
        $upAccessLevel->exeUpdate("sts_categories", $this->dados, "WHERE id =:id", "id={$this->dados['id']}");

        if ($upAccessLevel->getResult()) {
            $_SESSION['msg'] = "<div class='alert alert-success' role='alert'>Categoria da Noticia editada com sucesso!</div>";
            $this->resultado = true;
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Categoria da Noticia não editada com sucesso!</div>";
            $this->resultado = false;
        }
    }
