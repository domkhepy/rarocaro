...
/**
 * A classe AdmsListCategories Recebe as informações do nível de acesso que será listada na View
 *
 * @author Celke
 */
class AdmsListCategories 
{
    ...
    /** Metodo buscar as informações na tabela adms_categories e fazer a paginação do resultado que será mostrado na View listar nível de acesso
     * 
     * @param $pag Retorna a páginação
     */
    public function listCategories($pag = null) {
        
        $this->pag = (int) $pag;
        $paginacao = new \App\adms\Models\helper\AdmsPagination(URLADM . 'list-categories/index');
        $paginacao->condition($this->pag, $this->limitResult);
        $paginacao->pagination("SELECT COUNT(id) AS num_result FROM sts_categories ");
        $this->resultPg = $paginacao->getResult();

        $listCategories = new \App\adms\Models\helper\AdmsRead();
        $listCategories->fullRead("SELECT id, name
                FROM sts_categories
                LIMIT :limit OFFSET :offset", "limit={$this->limitResult}&offset={$paginacao->getOffset()}");

        $this->resultadoBd = $listCategories->getResult();
        if ($this->resultadoBd) {
            $this->resultado = true;
        } else {
