...
/**
 * A classe AdmsEditProductImage Recebe a informação da imagem do usuário que será editada no banco de dados
 *
 * @author Celke
 */
class AdmsEditProductImage
{
    ...
    public function viewProduct($id) {
        ...
        $viewProduct->fullRead("SELECT usu.id, usu.image
                FROM sts_products usu
                WHERE usu.id=:id 
    ...
    private function edit() {
        ...
        $upProduct = new \App\adms\Models\helper\AdmsUpdate();
        $upProduct->exeUpdate("sts_products", $this->saveDate, "WHERE id =:id", "id={$this->dados['id']}");

        if ($upProduct->getResult()) {
            $this->deleteImage();
        } else {
