...
/**
 * A classe StsEditContactPageImg
 *
 * @author Domingos
 */
class StsEditContactPageImg
{
    private $resultadoBd;
    ...
    private function upload() {
        ...
        $this->diretorio = "app/sts/assets/image/contact_page/" . $this->dados['id'] . "/";
    ...
    private function edit() {
        ...
        $upAboutsComp->exeUpdate("sts_contacts", $this->saveDate, "WHERE id =:id", "id={$this->dados['id']}");

        if ($upAboutsComp->getResult()) {
            $this->deleteImage();
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>Erro: Imagem Contacto não editada com sucesso!</div>";
            $this->resultado = false;
        }
    }
    ...
    private function deleteImage() {
        if ((!empty($this->resultadoBd[0]['image']) OR ($this->resultadoBd[0]['image'] != null)) AND ($this->resultadoBd[0]['image'] != $this->nameImg)) {
            $this->delImg = "app/sts/assets/image/contact_page/" . $this->dados['id'] . "/" . $this->resultadoBd[0]['image'];
            if (file_exists($this->delImg)) {
                unlink($this->delImg);
            }
        }

        $_SESSION['msg'] = "<div class='alert alert-success' role='alert'>Imagem Contacto editada com sucesso!</div>";
        $this->resultado = true;
    }
